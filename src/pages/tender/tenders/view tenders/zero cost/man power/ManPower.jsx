...
import { FiEdit2, FiSave, FiX, FiUsers, FiUserCheck } from "react-icons/fi";
...
const ManPower = () => {
  ...
  const [activeTab, setActiveTab] = useState("nmr");
  ...
  const fetchMachines = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error(`Error fetching ${activeTab.toUpperCase()} data`);
    } finally {
      setLoading(false);
    }
  }, [tender_id, activeTab]);
  ...
  const handleSave = async () => {
    try {
      ...
      if (res.status === 200) {
        toast.success(`${activeTab === 'nmr' ? 'NMR' : 'Contractor'} updated successfully`);
        setIsEditing(false);
        fetchMachines();
      }
    } catch (err) {
    ...
    <div className="flex flex-col gap-4 p-1 relative h-full">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded shadow-sm flex flex-col sm:flex-row sm:items-center justify-between p-4 gap-4">
        ...
        <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
          <button
            onClick={() => !isEditing && setActiveTab("nmr")}
            disabled={isEditing}
          ...
          >
            <FiUsers size={14} className={activeTab === "nmr" ? "text-blue-500" : "opacity-50"} />
            NMR (Nominal Muster Roll)
          </button>
          <button
            onClick={() => !isEditing && setActiveTab("contractor")}
            disabled={isEditing}
            className={`flex items-center gap-2 text-xs font-semibold px-4 py-2 rounded-md transition-all duration-200 disabled:cursor-not-allowed ${
          ...
          >
            <FiUserCheck size={14} className={activeTab === "contractor" ? "text-orange-500" : "opacity-50"} />
            Contractor
          </button>
        </div>
      ...
      <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded shadow-sm overflow-hidden flex flex-col grow">
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700">
          <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider">
            {activeTab === 'nmr' ? 'NMR Workforce' : 'Contractor Details'}
          </h3>
                ...
                <tr>
                  <td colSpan="9" className="px-4 py-12 text-center text-slate-500 dark:text-slate-400">
                    <div className="flex flex-col items-center justify-center gap-2">
                      {activeTab === 'nmr' ? <FiUsers size={24} className="opacity-20" /> : <FiUserCheck size={24} className="opacity-20" />}
                      <p>No manpower records found for this category.</p>
                    </div>
                  </td>
                </tr>
              ) : (
        ...
        <div className="bg-slate-100 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 px-6 py-4 flex flex-col sm:flex-row items-end sm:items-center justify-between gap-4 mt-auto">
           <div className="text-xs text-slate-500 dark:text-slate-400">
             Values automatically calculate Tax based on the updated Unit Rate.
           </div>
...
/**
 * Animated skeleton row for seamless loading UX (9 Columns)
 */
const SkeletonRow = () => (
