...
const fetchPlanDocuments = async ({ queryKey }) => {
  ...
  const { data } = await api.get(`/document/alldocuments/${tender_id}`);
...
const uploadPlansApi = async ({ tender_id, uploaded_by, files, onUploadProgress }) => {
  ...
  const endpoint = files.length === 1 ? "/document/upload" : "/document/upload-multiple";
...
export const useDeletePlan = () => {
  ...
  return useMutation({
    mutationFn: ({ document_id }) => api.delete(`/document/${document_id}`),
    onSuccess: () => {
