...
const UploadBid = ({ onclose, onSuccess }) => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        setUploadComplete(false); // Reset for potential next use
        if (onclose) onclose();
          ...
          <div className="absolute inset-0 z-10 bg-white/97 dark:bg-slate-900/97 backdrop-blur-sm flex flex-col items-center justify-center gap-5 px-8 animate-in fade-in duration-300">
            ...
            <div className="text-center">
              <p className="text-sm font-bold text-slate-700 dark:text-slate-200">
                ...
                {uploadProgress >= 20 && uploadProgress < 50 && "Uploading Boq…"}
        ...
        >
          ...
          <div className="mb-6 group">
            ...
            <input
              ...
              placeholder="e.g. 18"
              className="w-full px-4 py-3.5 bg-gray-50 dark:bg-slate-800/50 border border-gray-100 dark:border-slate-700 rounded-2xl outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all font-bold text-slate-800 dark:text-white shadow-sm"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mt-8">
            ...
            >
              Upload Bid {files.length > 0 && `(${files.length})`}
            </button>
          </div>
        </form>
      </div>
