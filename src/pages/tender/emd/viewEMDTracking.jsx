  ...
  <div className="w-full mt-8 bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden animate-pulse">
    ...
    {[1, 2, 3, 4, 5].map((row) => (
      <div key={row} className="p-4 border-b border-gray-50 flex gap-6 items-center">
        <div className="h-3 bg-gray-100 rounded w-1/4"></div> {/* Note */}
        <div className="h-3 bg-gray-100 rounded w-1/6"></div> {/* Amount Collected */}
        <div className="h-3 bg-gray-100 rounded w-1/6"></div> {/* Pending Amount */}
        <div className="h-3 bg-gray-100 rounded w-1/6"></div> {/* Date */}
        <div className="h-3 bg-gray-100 rounded w-1/6"></div> {/* Time */}
      </div>
    ))}
  </div>
);
...
const ErrorState = ({ message, onRetry }) => (
  <div className="flex flex-col items-center justify-center p-16 mt-8 bg-red-50 border border-red-100 rounded-xl">
    ...
    <button onClick={onRetry} className="mt-6 px-5 py-2 text-sm font-medium text-red-700 bg-white border border-red-300 rounded-lg transition-colors hover:bg-red-50">
      Try Again
    </button>
  </div>
);
...
const EMDTrackingTable = () => {
  ...
  const { data: tracking = [], isLoading, isError, refetch } = useEMDTracking(tender_id);
  ...
  if (tracking.length === 0) {
    return (
      <EmptyState 
        ...
        message={`We couldn't find any EMD collection records for Tender ID: ${tender_id}.`}
        onAction={() => navigate(-1)} 
      />
    );
  }
